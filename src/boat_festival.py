"""端午节抽奖活动自动化模块（浏览器自动化版）

活动页面: https://activity.zaimanhua.com/boat-festival/
任务: 分享活动、观看漫画、祝福评论
完成后抽奖，打印抽奖结果
...
def load_activity_page(page):
    ...
    page.goto(f"{BASE_URL}/boat-festival/?_={cache_buster}", wait_until="domcontentloaded", timeout=60000)
...
def get_draw_count(page) -> int:
    """获取当前抽奖次数"""
    try:
        page.goto(f"{BASE_URL}/boat-festival/", wait_until="domcontentloaded", timeout=60000)
        page.wait_for_timeout(3000)
...
def run_boat_festival(cookie_str: str, account_name: str):
    """单账号端午节活动流程（浏览器自动化版）"""
    print(f"\n  === 开始端午节抽奖活动 ===")
...
def main():
    ...
    for index, (account_name, cookie_str) in enumerate(cookies_list):
        ...
        success = run_boat_festival(cookie_str, account_name)
        if not success:
            all_success = False
